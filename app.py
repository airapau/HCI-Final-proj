...
import whisper                         # For audio-to-text transcription
from tempfile import NamedTemporaryFile  # For creating a temporary audio file
from transformers import pipeline      # For summarizer & sentiment models
from sentence_transformers import SentenceTransformer, util # For chatbot embeddings
import json                            # For reading Ollama's response
...
# --- CUSTOM CSS ---
# This block injects custom CSS for styling the app
st.markdown("""
<style>
    /* Main background */
    .stApp {
...
# --- SESSION STATE ---
# Initialize app memory to store data between reruns
...
# Functions decorated with @st.cache_resource only run once
...
def ollama_generate(prompt, model_name="deepseek-r1:1.5b-qwen-distill-q8_0"):
    ...
    try:
        ...
        output = ""
        # Piece together the streaming response
...
def extract_themes(text):
    ...
    # Prompt for Ollama to return a comma-separated list
...
def chatbot_response(query, transcript):
    """Generate chatbot response using semantic search (RAG)"""
    if not transcript:
        ...
        if preview_url:
            ...
            if audio_file:
                ...
                if transcribed_text:
                    ...
                    # Refresh the page to show the tabs
                    st.rerun()
        else:
...
# --- RESULTS DISPLAY ---
# Show this section only if conversion is complete
